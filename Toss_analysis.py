...
class ipl_analysis:
    def bar_grph(self,team_names, no_of_match_toss_win,no_of_match_indiv,Tot_indiv_mat_win,Year_input):
        ...
        result2=plt.bar(y_pos+bar_width,no_of_match_indiv,bar_width,color='r',alpha=1, label='Indiv_no_Match')
        ...
        plt.legend()
        plt.show()
    def pie_graph(self,totl_bat_field):
        label_field=['bat','field']
        plt.pie(totl_bat_field,labels=label_field,startangle=90,autopct='%.1f%%')
...
for read_1 in sorted(csv_data):
    ...
    if read_1['season']== Year_input:
        ...
        Toss_decision.append(read_1['toss_decision'])
Total_matches=len(Toss_won_team)
team_names=set(Toss_won_team)
totl_bat_field=[Toss_decision.count('bat'),Toss_decision.count('field')]
...
object_ipl.pie_graph(totl_bat_field)
